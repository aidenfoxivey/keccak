...
@cocotb.test()
async def test_keccak_random_input(dut):
    """Test with random input using Python reference."""
    cocotb.start_soon(Clock(dut.clk, 10, units="ns").start())
    await reset_dut(dut)
    ...
    for _ in range(10):
        ...
        for y in range(NUM_SHEETS):
            for x in range(NUM_PLANES):
                lane_val = 0
                for byte_idx in range(8):
                    lane_val |= initial_state_bytes[
                        (x + y * NUM_PLANES) * 8 + byte_idx
                    ] << (byte_idx * 8)
                dut.i_state[x][y].value = BinaryValue(
        ...
        assert dut.ready.value == 1, (
            "Operation should complete and ready should be high"
        )
        assert cycle_count == 25, (
        ...
        for y in range(NUM_SHEETS):
            for x in range(NUM_PLANES):
                if dut_output_matrix[y][x] != reference_output_matrix[x][y]:
                    raise TestFailure(
                        f"Mismatch at state[{x}][{y}]: DUT got {dut_output_matrix[y][x]}, "
                        f"expected {reference_output_matrix[x][y]}"
                    )
